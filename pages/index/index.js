...
Page({
  ...
  //领取优惠券
  lq:function(e){
    var url1 = app.globalData.comurl + "&m=Asset&a=ling", data11 = { token: app.globalData.token, id: e.target.dataset.id}, method = 'post', that = this;
  ...
  onShow: function () {
    ...
    //优惠券判断
    var url1 = app.globalData.comurl + "&m=Asset&a=first", data11 = { token: app.globalData.token}, method = 'post',that=this;
    app.ajaxData(url1, data11, method, function (res) {
      if (res.data.state == 10000){
        if (res.data.data!=''){
          that.setData({
    ...
    app.ajaxData(url, data, method, function (res) {
      ...
      if (res.data == 1) {
        ...
        if (option.openid_f != null && option.openid_f != 'undefined') {
          ...
          var url1 = app.globalData.comurl + "&m=Heroes&a=user_xinxi", data11 = { token: app.globalData.token, tel: app.globalData.tel, user_xinxi: JSON.stringify(app.globalData.userInfo), openid_f: wx.getStorageSync('openid_f') }, method = 'post';
          app.ajaxData(url1, data11, method, function (res) {
          ...
          var url = app.globalData.comurl + "&m=Heroes&a=index", data = { token: app.globalData.token }, method = 'post';
          app.ajaxData(url, data, method, function (res) {
            if (res.data.state == '10000') {
              if (res.data.data != null) {
