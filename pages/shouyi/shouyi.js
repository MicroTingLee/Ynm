...
Page({
  ...
  qh:function(e){
    ...
    if (this.data.xs == 0) {
      var that = this;
      // 我的推荐
      var url = app.globalData.comurl + "&m=Asset&a=tuijian", data = {token: app.globalData.token }, method = 'post';
      ...
      var that = this;
      //我的资产
      var url = app.globalData.comurl + "&m=Asset&a=mymoney", data = { token: app.globalData.token }, method = 'post';
      app.ajaxData(url, data, method, function (res) {
        console.log(res)
        if (res.data.state == 10000) {
          if (res.data.data != null) {
      ...
      var that = this;
      //经验值
      var url = app.globalData.comurl + "&m=Asset&a=index", data = { token: app.globalData.token }, method = 'post';
      app.ajaxData(url, data, method, function (res) {
        console.log(res)
        if (res.data.state == 10000) {
          if (res.data.data != null) {
